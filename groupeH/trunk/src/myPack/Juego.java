...
public class Juego {
	...
	void inicio() {
		...
		columnaFija = 0;
		valorFijo = 0;
		tableroFijo = new Tablero();
		tablero = new Tablero();
		dimension = Tablero.dimension;
		for (int z = 0; z < dimension * dimension; z++)
			jugadas[z] = new Celda(0, 0, 0);
		for (int x = 1; x <= dimension; x++)
			for (int y = 1; y <= dimension; y++)
				putJugadas(((x - 1) * 9) + y - 1, x - 1, y - 1, 0);
	}

	void putJugadas(int p, int f, int c, int v) {
		jugadas[p].putFila(f);
	...
	Tablero copyTablero(Tablero t) {
		...
		for (int fila = 0; fila < dimension; fila++)
			for (int columna = 0; columna < dimension; columna++)
				tt.casilla[fila][columna].putValor(t.casilla[fila][columna]
						.getValor());
		return tt;
	}
	...
	boolean incrementaPosicion() {
		...
		while (retorno) {
			if (Juego.filaFija < dimension - 1
					&& Juego.columnaFija < dimension - 1) {
				Juego.columnaFija++;
			} else if (Juego.filaFija == dimension - 1
					&& Juego.columnaFija < dimension - 1) {
				Juego.columnaFija++;
			} else if (Juego.columnaFija == dimension - 1
					&& Juego.filaFija < dimension - 1) {
				Juego.columnaFija = 0;
				Juego.filaFija++;
			} else {
				retorno = false;
			...
			if (tableroFijo.getValor(Juego.filaFija, Juego.columnaFija) < 1)
				break;
		}
		return retorno;
	}
	...
	boolean resolver() {
		...
		Juego.valorFijo = 0;
		activado = true;
		putTablero(tableroFijo);
		while (!tablero.completo() && !listaVacia()) {
			tablero = getTablero();
			Juego.valorFijo = 0;
			Juego.filaFija = tablero.getUFila();
			Juego.columnaFija = tablero.getUColumna();
			if (activado) {
				Juego.filaFija = 0;
				Juego.columnaFija = -1;
				Juego.valorFijo = 0;
			...
			if (incrementaPosicion()) {
				while (Juego.valorFijo < dimension) {
					Juego.valorFijo++;
					if (tablero.casillaValida(Juego.filaFija,
							Juego.columnaFija, Juego.valorFijo)) {
						tablero.putValor(Juego.filaFija, Juego.columnaFija,
								Juego.valorFijo);
						putTablero(tablero);
					}
				}
		...
		for (int fila = 0; fila < dimension; fila++) {
			for (int x = 0; x < listaTableros.size(); x++) {
				...
				for (int y = 0; y < dimension; y++)
					cadena = cadena + t.getValor(fila, y) + " ";
	...
	public static void main(String[] args) {
		...
		j.tableroFijo = new Tablero();
		j.tableroFijo.putValor(0, 0, 2);
		j.tableroFijo.putValor(1, 3, 1);
		j.tableroFijo.putValor(2, 1, 4);
		Juego.filaFija = 0;
		Juego.columnaFija = -1;
		Juego.valorFijo = 0;
		boolean activado = true;
		j.putTablero(j.tableroFijo);
		while (!t.completo() && !j.listaVacia()) {
			...
			if (j.incrementaPosicion() == true) {
				while (Juego.valorFijo < j.dimension) {
					Juego.valorFijo++;
					if (t.casillaValida(Juego.filaFija, Juego.columnaFija,
							Juego.valorFijo)) {
						t.putValor(Juego.filaFija, Juego.columnaFija,
								Juego.valorFijo);
						j.putTablero(t);

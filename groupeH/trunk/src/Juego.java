...
public class Juego{
  ...
  static int valorFijo;
	...
	void inicio() {
		...
		for(int x=1;x<=dimension;x++) for(int y=1;y<=dimension;y++) putJugadas(((x-1)*9)+y-1,x-1,y-1,0);
	}
	...
	boolean incrementaPosicion() {
			...
			while(retorno) {
				...
				if(tableroFijo.getValor(Juego.filaFija,Juego.columnaFija)<1) break;
			}
			return retorno;
	}
	...
	boolean resolver(){
		...
		while(!tablero.completo() && !listaVacia()) {
			...
			delTablero();
					...
					Juego.valorFijo++;
					if(tablero.casillaValida(Juego.filaFija,Juego.columnaFija,Juego.valorFijo)) {
                                          tablero.putValor(Juego.filaFija,Juego.columnaFija,Juego.valorFijo);
                                          putTablero(tablero);
					}
				}
			}
		...
		while(!t.completo() && !j.listaVacia()) {
			...
			if(j.incrementaPosicion()==true) {
				while(Juego.valorFijo<j.dimension) {
					Juego.valorFijo++;
					if(t.casillaValida(Juego.filaFija,Juego.columnaFija,Juego.valorFijo)) {
